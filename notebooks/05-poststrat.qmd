...
+ Próba nielosowa o wielkości $n_b=1000$ jest losowana proporcjonalnie do cechy $Y$, gdzie prawdopodobieństwo wylosowania danego rekordu określone jest w następujący sposób:
...
+ ocenic zależność między cechą X1 i X2
...
Wniosek: główna rozbieżność w zmiennej `X1`, w `X2` niezbyt.
...
boxplot((cbind(y_naive, y_ps) - mean(Y))/mean(Y)*100, xlab = "Estymator", y = "Relatywne obciążenie (w %)")
```

```{r}
c(mean(Y), mean(y_naive), mean(y_ps))
```
