---
title: "Metody quasi-randomizacyjne: propensity score"
format: 
  html:
    self-contained: true
...
dane |> 
  filter(R == TRUE) |>
  summarise(estymator_ps = weighted.mean(jedna_zmiana, waga_estym),
...
dane_ps <- dane |>
  ...
  mutate(total_ps = case_when(klasa_pr == "D" ~ (94124/total),
                              klasa_pr == "S" ~ 128940/total,
                              klasa_pr == "M" ~ 71230/total),
         waga_ps = waga_estym*total_ps) 
dane_ps
```
...
dane_ps |> 
  summarise(estymator_ps = weighted.mean(jedna_zmiana, waga_estym),
            estymator_ps2 = weighted.mean(jedna_zmiana, waga_ps),
            suma_wag = sum(waga_estym),
            suma_wag_ps = sum(waga_ps))
```
